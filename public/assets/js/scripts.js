...
const need_location = window.location.href;
need_url = need_location.replace(/.*\/\/[^\/]*/, '');
const profile = '/index.php?route=profile';
const login = '/index.php?route=login';
const register = '/index.php?route=register';

if (['/index.php?id=1', '/index.php?id=2', '/index.php?id=3'].includes(need_url)) {
// Toggle sidebar
...
} else if (login === need_url) {
    setTimeout(function () {
        document.getElementById('message').style.display = 'none';
    }, 3000);
} else if (register === need_url) {
    setTimeout(function () {
        document.getElementById('message').style.display = 'none';
    }, 3000);
} else if (profile === need_url) {
    // Toggle user menu
    userMenu.addEventListener('click', function () {
        userSettings.classList.toggle('open-user-menu');
    });
